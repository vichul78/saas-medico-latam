import { Link } from 'react-router-dom';
import {
  Stethoscope, Brain, FileImage, CalendarCheck,
  ShieldCheck, Globe, ArrowRight, Play, Sparkles,
  Zap, Users, BarChart3, ChevronRight,
} from 'lucide-react';

/**
 * LandingPage — página de inicio pública con diseño Liquid Glass.
 *
 * Estructura:
 *   1. Navbar liquid-glass (sticky)
 *   2. Hero a pantalla completa con video de fondo
 *      — texto abajo a la izquierda
 *   3. Sección de Características (tarjetas liquid-glass)
 *   4. Sección de Especialidades (grid)
 *   5. Sección CTA final
 *   6. Footer mínimo
 *
 * Reglas visuales ESTRICTAS:
 *   • Acentos: Púrpura Eléctrico (#7A22FF) y Violeta (#5B27B5).
 *   • CERO tonos verdes en ningún elemento.
 *   • Iconos: lucide-react.
 *   • Fuente: Geist (cargada en index.html).
 *   • Avatares/íconos de médicos: líneas limpias, sin vello facial.
 */

const VIDEO_SRC =
  'https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/hf_20260511_230229_7c9bc431-46cf-489a-948d-e8144d8eb5d4.mp4';

/* ── Datos de características ── */
const FEATURES = [
  {
    icon:  <Brain className="h-6 w-6" />,
    title: 'IA Clínica',
    desc:  'Copiloto Iris analiza estudios, genera informes estructurados y sugiere diagnósticos diferenciales.',
    accent: 'text-electric-300',
    ring:   'ring-electric-500/30',
  },
  {
    icon:  <FileImage className="h-6 w-6" />,
    title: 'Visor DICOM',
    desc:  'Soporte nativo CT · MR · DX · US. Herramientas de medición, WW/WL y multiplanar.',
    accent: 'text-violet-300',
    ring:   'ring-violet-500/30',
  },
  {
    icon:  <CalendarCheck className="h-6 w-6" />,
    title: 'Gestión de Citas',
    desc:  'Agenda multi-recurso, recordatorios automáticos y predicción de inasistencias.',
    accent: 'text-electric-300',
    ring:   'ring-electric-500/30',
  },
  {
    icon:  <ShieldCheck className="h-6 w-6" />,
    title: 'Seguridad LatAm',
    desc:  'Cumplimiento HIPAA · LGPD · LFPDPPP. Datos residentes en la región.',
    accent: 'text-violet-300',
    ring:   'ring-violet-500/30',
  },
  {
    icon:  <Globe className="h-6 w-6" />,
    title: 'Multi-moneda',
    desc:  'Facturación en MXN · BRL · ARS · COP · CLP y 7 monedas LatAm más.',
    accent: 'text-electric-300',
    ring:   'ring-electric-500/30',
  },
  {
    icon:  <Stethoscope className="h-6 w-6" />,
    title: '11 Especialidades',
    desc:  'Radiología, Dental, Cardiología, Neumología, Oftalmología y más.',
    accent: 'text-violet-300',
    ring:   'ring-violet-500/30',
  },
];

/* ── Estadísticas hero ── */
const STATS = [
  { value: '11',   label: 'Especialidades', icon: <Stethoscope className="h-4 w-4" /> },
  { value: '11',   label: 'Monedas LatAm',  icon: <Globe className="h-4 w-4" /> },
  { value: '100%', label: 'Nativo DICOM',   icon: <FileImage className="h-4 w-4" /> },
  { value: 'IA',   label: 'Copiloto Iris',  icon: <Brain className="h-4 w-4" /> },
];

export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#0a0a12] font-[Geist,Inter,sans-serif] text-white">

      {/* ══════════════════════════════════════════════════════════════
          NAVBAR — liquid-glass, sticky
      ══════════════════════════════════════════════════════════════ */}
      <nav
        className="liquid-glass fixed inset-x-0 top-0 z-50 rounded-none"
        style={{ borderRadius: 0 }}
      >
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">

          {/* Logo / nombre */}
          <Link to="/" className="flex items-center gap-2.5 group">
            {/* Ícono hexagonal inline SVG */}
            <div className="flex h-9 w-9 items-center justify-center rounded-xl
                            bg-gradient-to-br from-[#7A22FF] to-[#5B27B5]
                            shadow-[0_0_16px_-4px_rgba(122,34,255,0.7)]
                            transition group-hover:shadow-[0_0_24px_-2px_rgba(122,34,255,0.9)]">
              <Stethoscope className="h-5 w-5 text-white" />
            </div>
            <span className="font-[Geist] text-lg font-semibold tracking-tight text-white">
              MediCo<span className="text-[#9450FF]"> LatAm</span>
            </span>
          </Link>

          {/* Links de navegación — desktop */}
          <div className="hidden items-center gap-6 md:flex">
            {['Especialidades', 'Funcionalidades', 'Precios'].map(item => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="text-sm font-medium text-white/60 transition hover:text-white"
              >
                {item}
              </a>
            ))}
          </div>

          {/* Botones CTA */}
          <div className="flex items-center gap-3">
            {/* Iniciar Sesión → /login (ruta existente, NO modificada) */}
            <Link
              to="/login"
              className="liquid-glass rounded-xl px-4 py-2 text-sm font-medium
                         text-white/80 transition hover:text-white"
            >
              Iniciar Sesión
            </Link>

            {/* Comenzar → púrpura eléctrico, NUNCA verde */}
            <Link
              to="/contratar"
              className="inline-flex items-center gap-1.5 rounded-xl
                         bg-gradient-to-r from-[#7A22FF] to-[#5B27B5]
                         px-4 py-2 text-sm font-semibold text-white
                         shadow-[0_4px_20px_-6px_rgba(122,34,255,0.6)]
                         transition hover:brightness-110 active:brightness-95"
            >
              Comenzar
              <ChevronRight className="h-4 w-4" />
            </Link>
          </div>
        </div>
      </nav>

      {/* ══════════════════════════════════════════════════════════════
          HERO — pantalla completa, video de fondo, texto abajo-izquierda
      ══════════════════════════════════════════════════════════════ */}
      <section className="relative flex min-h-screen flex-col overflow-hidden">

        {/* Video de fondo */}
        <video
          src={VIDEO_SRC}
          autoPlay
          muted
          loop
          playsInline
          aria-hidden
          className="absolute inset-0 h-full w-full object-cover"
        />

        {/* Overlay oscuro para legibilidad */}
        <div className="absolute inset-0 bg-gradient-to-b
                        from-[#0a0a12]/60 via-[#0a0a12]/30 to-[#0a0a12]/80" />

        {/* Destellos decorativos */}
        <div aria-hidden
             className="pointer-events-none absolute -left-40 top-1/3 h-[500px] w-[500px]
                        rounded-full bg-[#7A22FF] opacity-[0.12] blur-[120px]" />
        <div aria-hidden
             className="pointer-events-none absolute -right-32 top-1/2 h-[400px] w-[400px]
                        rounded-full bg-[#5B27B5] opacity-[0.10] blur-[100px]" />

        {/* ── Contenido del hero ── */}
        <div className="relative z-10 flex flex-1 flex-col justify-end px-5 pb-20 sm:px-12 lg:px-20">

          {/* Badge eyebrow */}
          <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full
                          border border-[#7A22FF]/40 bg-[#7A22FF]/10
                          px-3 py-1 text-xs font-semibold uppercase
                          tracking-[0.16em] text-[#CFA8FF]">
            <Sparkles className="h-3.5 w-3.5" />
            Plataforma clínica para LatAm
          </div>

          {/* Título principal */}
          <h1 className="mb-5 max-w-3xl font-[Geist] text-5xl font-bold
                         leading-[1.05] tracking-tight text-white
                         sm:text-6xl lg:text-7xl">
            La Evolución del
            <br />
            <span className="bg-gradient-to-r from-[#9450FF] to-[#CFA8FF]
                             bg-clip-text text-transparent">
              Diagnóstico Radiológico
            </span>
          </h1>

          {/* Descripción */}
          <p className="mb-8 max-w-xl text-base leading-relaxed text-white/60 sm:text-lg">
            Gestiona pacientes, analiza imágenes DICOM y potencia tu clínica
            con Inteligencia Artificial diseñada para Latinoamérica.
          </p>

          {/* Botones de acción principal */}
          <div className="flex flex-wrap items-center gap-4">
            {/* CTA primario — eléctrico, NUNCA verde */}
            <Link
              to="/contratar"
              className="inline-flex items-center gap-2 rounded-2xl
                         bg-gradient-to-r from-[#7A22FF] to-[#5B27B5]
                         px-7 py-3.5 text-base font-semibold text-white
                         shadow-[0_8px_32px_-8px_rgba(122,34,255,0.65)]
                         transition hover:brightness-110 hover:scale-[1.02]
                         active:scale-[0.98]"
            >
              <Zap className="h-5 w-5" />
              Comenzar gratis
            </Link>

            {/* CTA secundario — glass */}
            <Link
              to="/funcionalidades/tecnologia/visor-dicom"
              className="liquid-glass inline-flex items-center gap-2 rounded-2xl
                         px-7 py-3.5 text-base font-semibold text-white
                         transition hover:bg-white/[0.08]"
            >
              <Play className="h-5 w-5 fill-current" />
              Ver demo DICOM
            </Link>
          </div>

          {/* Estadísticas inline */}
          <div className="mt-10 flex flex-wrap gap-4">
            {STATS.map(s => (
              <div
                key={s.label}
                className="liquid-glass flex items-center gap-2.5 rounded-2xl px-4 py-2.5"
              >
                <span className="text-[#9450FF]">{s.icon}</span>
                <span className="font-[Geist] text-xl font-bold text-white">{s.value}</span>
                <span className="text-xs text-white/50">{s.label}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Flecha scroll-down */}
        <div className="relative z-10 flex justify-center pb-6 animate-bounce">
          <div className="h-6 w-6 rounded-full border border-white/20 flex items-center
                          justify-center text-white/40">
            <ChevronRight className="h-4 w-4 rotate-90" />
          </div>
        </div>
      </section>

      {/* ══════════════════════════════════════════════════════════════
          CARACTERÍSTICAS — tarjetas liquid-glass
      ══════════════════════════════════════════════════════════════ */}
      <section id="funcionalidades" className="relative py-24 px-5 sm:px-8">
        {/* Fondo sutil */}
        <div aria-hidden
             className="pointer-events-none absolute inset-0
                        bg-gradient-to-b from-[#0a0a12] via-[#0f0a1e] to-[#0a0a12]" />

        <div className="relative z-10 mx-auto max-w-7xl">
          {/* Cabecera de sección */}
          <div className="mb-14 text-center">
            <p className="mb-3 inline-flex items-center gap-2 rounded-full
                          border border-[#7A22FF]/30 bg-[#7A22FF]/10
                          px-3 py-1 text-xs font-semibold uppercase
                          tracking-[0.16em] text-[#9450FF]">
              <Sparkles className="h-3 w-3" />
              Todo en una plataforma
            </p>
            <h2 className="font-[Geist] text-4xl font-bold tracking-tight text-white
                           sm:text-5xl">
              Tecnología clínica de
              <span className="bg-gradient-to-r from-[#9450FF] to-[#CFA8FF]
                               bg-clip-text text-transparent"> primer nivel</span>
            </h2>
            <p className="mx-auto mt-4 max-w-2xl text-base text-white/50">
              Diseñado desde cero para los flujos de trabajo reales de clínicas
              y hospitales en México, Brasil, Colombia y toda la región.
            </p>
          </div>

          {/* Grid de tarjetas */}
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map(f => (
              <FeatureCard key={f.title} feature={f} />
            ))}
          </div>
        </div>
      </section>

      {/* ══════════════════════════════════════════════════════════════
          ESPECIALIDADES — grid pill
      ══════════════════════════════════════════════════════════════ */}
      <section id="especialidades" className="py-20 px-5 sm:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <h2 className="font-[Geist] text-3xl font-bold tracking-tight text-white sm:text-4xl">
              11 especialidades, una sola plataforma
            </h2>
          </div>

          <div className="flex flex-wrap justify-center gap-3">
            {[
              'Radiología', 'Dental', 'Cirugía', 'Cardiología', 'Neumología',
              'Audiometría', 'Patología', 'Obstétrico', 'Colposcopia',
              'Oftalmología', 'Veterinaria',
            ].map(spec => (
              <span
                key={spec}
                className="liquid-glass rounded-full px-5 py-2.5 text-sm
                           font-medium text-white/80 transition
                           hover:text-white hover:bg-white/[0.06]"
              >
                {spec}
              </span>
            ))}
          </div>
        </div>
      </section>

      {/* ══════════════════════════════════════════════════════════════
          CTA FINAL
      ══════════════════════════════════════════════════════════════ */}
      <section className="relative overflow-hidden py-28 px-5 sm:px-8">
        {/* Glow de fondo */}
        <div aria-hidden
             className="pointer-events-none absolute left-1/2 top-1/2
                        h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2
                        rounded-full bg-[#7A22FF] opacity-[0.12] blur-[140px]" />

        <div className="relative z-10 mx-auto max-w-3xl text-center">
          <h2 className="mb-5 font-[Geist] text-4xl font-bold tracking-tight
                         text-white sm:text-5xl">
            Tu clínica, potenciada con IA.
          </h2>
          <p className="mb-10 text-lg text-white/50">
            Únete a las clínicas de LatAm que ya digitalizaron su operación con
            MediCo LatAm. Sin papeles, sin demoras.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <Link
              to="/contratar"
              className="inline-flex items-center gap-2 rounded-2xl
                         bg-gradient-to-r from-[#7A22FF] to-[#5B27B5]
                         px-8 py-4 text-base font-semibold text-white
                         shadow-[0_8px_40px_-8px_rgba(122,34,255,0.7)]
                         transition hover:brightness-110 hover:scale-[1.02]
                         active:scale-[0.98]"
            >
              <ArrowRight className="h-5 w-5" />
              Comenzar ahora
            </Link>
            <Link
              to="/login"
              className="liquid-glass inline-flex items-center gap-2 rounded-2xl
                         px-8 py-4 text-base font-semibold text-white
                         transition hover:bg-white/[0.08]"
            >
              <Users className="h-5 w-5" />
              Ya tengo cuenta
            </Link>
          </div>
        </div>
      </section>

      {/* ══════════════════════════════════════════════════════════════
          FOOTER
      ══════════════════════════════════════════════════════════════ */}
      <footer className="border-t border-white/[0.06] py-8 px-5 sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-col items-center
                        justify-between gap-4 sm:flex-row">
          <p className="text-sm text-white/30">
            © {new Date().getFullYear()} MediCo LatAm · Plataforma clínica regional
          </p>
          <div className="flex items-center gap-4 text-sm text-white/30">
            {['HIPAA', 'LGPD', 'LFPDPPP'].map(b => (
              <span key={b}
                    className="rounded-full border border-[#5B27B5]/30
                               bg-[#5B27B5]/10 px-2 py-0.5 text-[10px]
                               font-semibold uppercase tracking-wider text-[#9450FF]">
                {b}
              </span>
            ))}
          </div>
          <div className="flex gap-4 text-sm text-white/30">
            <BarChart3 className="h-4 w-4 text-[#7A22FF]/50" />
            <span>Build: {new Date().toLocaleDateString('es')}</span>
          </div>
        </div>
      </footer>

    </div>
  );
}

/* ── Tarjeta de característica ── */
function FeatureCard({ feature: f }) {
  return (
    <div className={`liquid-glass group rounded-2xl p-6 ring-1 ${f.ring}
                     transition hover:ring-2 hover:bg-white/[0.04]`}>
      <div className={`mb-4 inline-flex h-11 w-11 items-center justify-center
                       rounded-xl bg-white/[0.06] ${f.accent}
                       ring-1 ${f.ring} transition group-hover:scale-110`}>
        {f.icon}
      </div>
      <h3 className="mb-2 font-[Geist] text-base font-semibold text-white">
        {f.title}
      </h3>
      <p className="text-sm leading-relaxed text-white/50">{f.desc}</p>
    </div>
  );
}
