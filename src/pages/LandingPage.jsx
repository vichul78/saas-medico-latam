...
/* ═══════════════════════════════════════════════════════════════════════════════
   ...
   - Full-screen: w-full min-h-screen bg-black
      ...
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <button type="button" onClick={() => setMobileOpen(v => !v)}
          className="flex h-10 w-10 items-center justify-center rounded-xl
                     border border-white/[0.08] text-white/70 lg:hidden" aria-label="Menú">
...
function FuncionalidadesDropdown() {
  ...
  return (
    <div ref={ref} className="relative"
      ...
      <div className={`absolute left-1/2 top-full z-50 mt-3 -translate-x-1/2 transition-all duration-200
        ${open ? 'pointer-events-auto translate-y-0 opacity-100' : 'pointer-events-none -translate-y-2 opacity-0'}`}>
        <div className="w-[600px] rounded-2xl border border-white/[0.08] bg-black/95 backdrop-blur-2xl p-5
                        shadow-[0_24px_64px_-12px_rgba(0,0,0,0.9),0_0_0_1px_rgba(122,34,255,0.08)]">
                ...
                <div className="space-y-0.5">
                  {block.items.map(item => (
                    <Link key={item.href} to={item.href} onClick={() => setOpen(false)}
                      className="block rounded-lg px-3 py-1.5 text-sm text-white/70
                                 transition hover:bg-[#7A22FF]/10 hover:text-white">
                      {item.label}
                    </Link>
                  ))}
...
function Hero() {
  return (
    <section className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 pt-28 pb-16">
      <div aria-hidden className="pointer-events-none absolute left-1/2 top-1/4 h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#7A22FF] opacity-[0.06] blur-[160px]" />
      <div aria-hidden className="pointer-events-none absolute right-1/4 bottom-1/4 h-[400px] w-[400px] rounded-full bg-[#5B27B5] opacity-[0.05] blur-[120px]" />
      ...
      <h1 className="mb-6 max-w-4xl text-center text-5xl font-bold leading-[1.05] tracking-tight text-white sm:text-6xl lg:text-[72px]">
      ...
      <p className="mb-10 max-w-2xl text-center text-lg leading-relaxed text-white/50">
...
function BrowserMockup() {
  return (
    <div className="w-full max-w-6xl">
      <div className="rounded-2xl border border-white/[0.10] bg-white/[0.02] backdrop-blur-xl
                      shadow-[0_32px_80px_-16px_rgba(0,0,0,0.8),0_0_0_1px_rgba(122,34,255,0.1)] overflow-hidden">
        <div className="flex h-10 items-center gap-2.5 border-b border-white/[0.07] bg-black/60 px-4">
          ...
          <div className="mx-auto flex h-6 w-64 items-center gap-2 rounded-md border border-white/[0.08] bg-white/[0.03] px-3">
            <div className="h-2 w-2 rounded-full bg-[#7A22FF]/50" />
            <span className="text-[11px] font-mono text-white/35">app.medicolatam.com/visor</span>
          </div>
        </div>
        <div className="relative flex h-[400px] overflow-hidden sm:h-[480px]">
          {/* Series panel */}
          <div className="hidden w-36 shrink-0 border-r border-white/[0.05] bg-black/50 p-3 sm:block">
            ...
            {['TC Tórax Axial', 'TC Tórax Coronal', 'Rx PA'].map((s, i) => (
              <div key={s} className={`mb-1.5 rounded-lg p-2 text-[10px] text-white/50
                ${i === 0 ? 'border border-[#7A22FF]/40 bg-[#7A22FF]/12 text-white/80' : ''}`}>
          ...
          <div className="relative flex flex-1 items-center justify-center bg-black">
            ...
            <div className="pointer-events-none absolute left-3 top-3 font-mono text-[9px] text-white/35">
              <p>PACIENTE: Demo · García</p><p>CT · Axial</p>
            </div>
            <div className="pointer-events-none absolute right-3 top-3 text-right font-mono text-[9px] text-white/35">
              <p>WW/WL: 400/40</p><p>IMG: 24/48</p>
            </div>
          </div>
          {/* Copilot panel */}
          <div className="hidden w-52 shrink-0 border-l border-white/[0.05] bg-[#050010]/80 sm:flex sm:flex-col">
            ...
            <div className="flex-1 space-y-2 p-3">
              <div className="rounded-lg bg-white/[0.04] px-2.5 py-1.5 text-[10px] text-white/50">Analizando TC de tórax…</div>
              <div className="ml-2 rounded-lg border border-[#7A22FF]/25 bg-[#7A22FF]/8 px-2.5 py-1.5 text-[10px] text-white/60">Parénquima sin consolidaciones. CTR normal.</div>
            </div>
          </div>
        </div>
      </div>
...
function ProductTabs() {
  ...
  return (
    <section className="relative py-28 px-4 sm:px-8">
      <div aria-hidden className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black via-[#06001a] to-black" />
      <div className="relative z-10 mx-auto max-w-6xl">
            ...
            <button key={t.id} type="button" onClick={() => setActive(t.id)}
              className={`flex items-center gap-2 rounded-xl border px-6 py-3 text-sm font-semibold transition
                ${active === t.id
                  ...
                  : 'border-white/[0.07] bg-white/[0.02] text-white/45 hover:border-[#7A22FF]/30 hover:text-white/75'}`}>
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <p className="mb-8 text-base leading-relaxed text-white/50">{tab.desc}</p>
            <ul className="space-y-3">
              {tab.points.map(pt => (
                <li key={pt} className="flex items-start gap-3">
                  <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[#9450FF]" />
                  <span className="text-sm text-white/65">{pt}</span>
                </li>
              ))}
            </ul>
            <div className="mt-10">
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-gradient-to-br from-black to-[#0a0020] p-6 shadow-[0_20px_50px_-10px_rgba(0,0,0,0.7)] ring-1 ring-[#7A22FF]/8">
...
function TabVisual({ tabId }) {
  if (tabId === 'pacs') return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 mb-3"><Monitor className="h-5 w-5 text-[#7A22FF]" /><span className="text-sm font-semibold text-white">Visor DICOM</span>
      ...
      <div className="flex gap-2">{['Pulmón','Hueso','Tejido','Cerebro'].map(p => (
        <div key={p} className="flex-1 rounded-lg border border-white/[0.07] bg-white/[0.03] py-1.5 text-center text-[10px] font-medium text-white/45">{p}</div>
      ))}</div>
    </div>
  );
  if (tabId === 'billing') return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 mb-2"><Receipt className="h-5 w-5 text-[#5B27B5]" /><span className="text-sm font-semibold text-white">Dashboard Financiero</span></div>
      {[{ label: 'Ingresos del mes', val: '$124,500 MXN', pct: 78, c: '#7A22FF' },
        ...
        <div key={r.label} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
          <div className="mb-1.5 flex items-center justify-between"><p className="text-xs text-white/45">{r.label}</p><p className="text-sm font-bold text-white">{r.val}</p></div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.07]"><div className="h-full rounded-full" style={{ width: `${r.pct}%`, background: r.c }} /></div>
        </div>
      ))}
    </div>
  );
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 mb-3"><Brain className="h-5 w-5 text-[#9450FF]" /><span className="text-sm font-semibold text-white">Copiloto Iris</span>
      ...
      {[{ who: 'iris', text: 'Listo para analizar estudios y dictar informes.' },
        ...
        <div key={i} className={`rounded-xl px-3 py-2 text-[11px] leading-relaxed
          ${m.who === 'iris' ? 'bg-white/[0.04] text-white/60' : 'ml-5 border border-[#7A22FF]/25 bg-[#7A22FF]/8 text-white/70'}`}>{m.text}</div>
      ))}
    </div>
  );
}
    ...
    <section className="relative py-24 px-4 sm:px-8">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(f => (
            <div key={f.title} className="group rounded-2xl border border-white/[0.06] bg-white/[0.015] p-6 backdrop-blur-sm transition
                         hover:border-[#7A22FF]/30 hover:bg-[#7A22FF]/[0.03] hover:shadow-[0_0_36px_-8px_rgba(122,34,255,0.2)]">
              <div className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl bg-white/[0.05] text-[#9450FF]
    ...
    <section className="relative py-20 px-4 sm:px-8">
      <div className="relative z-10 mx-auto max-w-5xl">
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/[0.06] md:grid-cols-4">
          {STATS.map((s, i) => (
            <div key={s.label} className={`flex flex-col items-center justify-center py-10 bg-white/[0.015] ${i < STATS.length - 1 ? 'border-r border-white/[0.04]' : ''}`}>
...
function CTASection() {
  return (
    <section className="relative overflow-hidden py-28 px-4 sm:px-8">
      ...
      <div className="relative z-10 mx-auto max-w-4xl overflow-hidden rounded-3xl border border-[#7A22FF]/20 bg-white/[0.015] p-14 text-center backdrop-blur-sm shadow-[0_0_80px_-20px_rgba(122,34,255,0.2)]">
        ...
        <p className="mx-auto mb-10 max-w-xl text-lg text-white/45">Sin papeles, sin demoras. Únete a las clínicas de LatAm que ya digitalizaron su operación completa.</p>
...
function Footer() {
  return (
    <footer className="border-t border-white/[0.05] py-8 px-4 sm:px-8">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 sm:flex-row">
        <p className="text-sm text-white/25">© {new Date().getFullYear()} MediCo LatAm · Plataforma clínica regional</p>
